...
if __name__ == '__main__':
    ...
    def category_translation(category_list):
        ...
        # 定义品类列表：
        category_dict = {'01': '奶粉', '02': '营养保健', '03': '尿布尿裤', '04': '童装童鞋', '05': '喂养用品', '06': '洗护用品', '07': '车床出行',
                         '08': '玩具文娱', '09': '妈妈用品', '10': '婴儿棉品', '11': '辅食零食', '18': '服务类', '90': '其他', '91': '折扣类',
                         '93': '非销售（不计入大盘销售）'}
    ...
    # 奖励类型映射
    def prize_type_translation(order_grade):
        if order_grade['type'] == 'coupon':
            prize_type = '优惠券'
            serial_number = order_grade['number']
            serial_no = order_grade['serial_no']
            expiration_date_type = order_grade['expiration_date_type']
            if expiration_date_type == 'by_day':
                begin_date = "奖励发放当天起"
                end_date = order_grade['expiration_day']

            else:
                begin_date = order_grade['begin_date']
                end_date = order_grade['end_date']
        elif order_grade['type'] == 'choice':
            prize_type, serial_number, serial_no, begin_date, end_date=('设置有误','设置有误','设置有误','设置有误','设置有误')
        elif order_grade['type'] == 'promotion':
            prize_type, serial_number, serial_no, begin_date, end_date=('设置有误','设置有误','设置有误','设置有误','设置有误')
        elif order_grade['type'] == 'promotion':
            prize_type, serial_number, serial_no, begin_date, end_date=('设置有误','设置有误','设置有误','设置有误','设置有误')
    ...
    if activity_order_type == 'order':
        ...
        activity_order_condition_type, order_grade_flag = check_activity_order_condition_type_first(
            activity_order_condition['type'])
        # 参与的奖励条码
        ...
        activity_order_type = '单内商品'
        activity_order_condition_type, order_grade_flag = check_activity_order_condition_type(
            activity_order_condition['type'])
    ...
    if order_grade_flag == 1:
        order_garde_str = ''
        order_grade_list = []
        for order_grade, order_grade_values in prize_items_range.items():
            order_grade_list.append(order_grade)
            print(order_grade_values)
            prize_type, serial_number, serial_no, serial_begin_date, serial_end_date = prize_type_translation(
                order_grade_values[0])
            order_garde_str += f'''等级{order_grade}：{order_grade}；券号：{serial_no};发放数量：{serial_number}；开始时间：{serial_begin_date}；结束时间：{serial_end_date}'''
